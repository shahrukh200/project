resource "azurerm_resource_group" "hub1" {
...
resource "azurerm_virtual_network" "vnet001" {
    ...
    resource_group_name = azurerm_resource_group.hub1.name
    location = azurerm_resource_group.hub1.location
    depends_on = [ azurerm_resource_group.hub1 ]
}
...
resource "azurerm_firewall_policy" "firewall_policy" {
  ...
  resource_group_name = azurerm_resource_group.hub1.name
  sku = "Standard"
  depends_on = [ azurerm_resource_group.hub1, azurerm_subnet.subnet ]
}
...
resource "azurerm_firewall" "firewall001" {
  ...
  depends_on = [ azurerm_resource_group.hub1 , azurerm_public_ip.public_ips , 
...
resource_group_name= azurerm_resource_group.hub1.name
location=azurerm_resource_group.hub1.location
cidrs=["10.3.0.0/16" ,"10.4.0.0/16" ,"10.5.0.0/16"]
depends_on = [ azurerm_resource_group.hub1 ]
}
...
resource "azurerm_virtual_network_gateway" "gateway" {
  ...
  depends_on = [ azurerm_resource_group.hub1 , azurerm_public_ip.public_ips , azurerm_subnet.subnet]
}
...
data "azurerm_virtual_network" "on-prem_vnet" {
  name = "vnet001"
  resource_group_name = "on-prem"
}
...
resource "azurerm_local_network_gateway" "Hub_local_gateway" {
  ...
  address_space       = [data.azurerm_virtual_network.on-prem_vnet.address_space[0]] 

  depends_on = [ azurerm_public_ip.public_ips , azurerm_virtual_network_gateway.gateway , 
              data.azurerm_public_ip.public_ips ,data.azurerm_virtual_network.on-prem_vnet ]
}
